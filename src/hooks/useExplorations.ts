...
export default ({ params = {} }) => {
  ...
  const [createMutation, execCreateMutation] = useMutation(
    newExplorationMutation,
    { role }
  );
  const [genSqlMutation, execGenSqlMutation] = useMutation(
    genExplorationSqlMutation,
    { role }
  );
  ...
  const current = useMemo(
    () => currentData.data?.explorations_by_pk || {},
    [currentData.data]
  );
